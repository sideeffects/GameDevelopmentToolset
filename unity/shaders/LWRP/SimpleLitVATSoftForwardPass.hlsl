...
Varyings LitPassVertexSimple(Attributes input)
{
    ...
    VATAttributes outputVAT = GetAttributesFromTexture(TEXTURE2D_PARAM(_posTex, sampler_posTex), 
                    TEXTURE2D_PARAM(_nTex, sampler_nTex), TEXTURE2D_PARAM(_colorTex, sampler_colorTex),
...
half4 LitPassFragmentSimple(Varyings input) : SV_Target
{
    ...
    half3 diffuse = diffuseAlpha.rgb * _Color.rgb * input.color.rgb;
    // diffuse = input.color;
    ...
    // float bbox = input.color.a;
    // seed = seed * 0.2 + 0.8;
    // float rampPos = input.color.r + input.color.g;
    // rampPos *= bbox;
    // rampPos *= seed;
    // rampPos /= 1.3;
    // rampPos += input.custom.y;
    // rampPos = clamp(rampPos, 0.01, 0.99);
    // emission = SampleRamp(rampPos, TEXTURE2D_PARAM(_RampTex, sampler_RampTex));
